...
best_model_name <- best_bayes_elpd$Model
best_family <- best_bayes_elpd$Family
best_structure <- best_bayes_elpd$Structure

if (best_family == "lognormal" && best_structure == "Fixed") {
  best_fit <- lognormal_fixed_results[[best_model_name]]$Fit
} else if (best_family == "lognormal" && best_structure == "Random earthquake") {
  best_fit <- lognormal_random_results[[best_model_name]]$Fit
} else if (best_family == "weibull" && best_structure == "Fixed") {
  best_fit <- weibull_fixed_results[[best_model_name]]$Fit
} else if (best_family == "weibull" && best_structure == "Random earthquake") {
  best_fit <- weibull_random_results[[best_model_name]]$Fit
}
...
ggsave(
  filename = "M9_MCMC_diagnostics.png",
  plot = combined_plot,
  width = 13,
  height = 15,
  dpi = 300
...
# Best models
best_full_fit <- bayesian_aft_results$weibull_random[["M9"]]$Fit
...
# Appearing only in M9 with magnitude/depth
  ...
  function(variable) {
    bind_rows(
      extract_conditional_effect(
        ...
        variable = variable,
        model_name = "With magnitude/depth (M9)"
      ),
      extract_conditional_effect(
